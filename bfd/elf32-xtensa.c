...
static reloc_howto_type elf_howto_table[] =
{
  ...
  /* FDPIC relocations.  FUNCDESC, FUNCDESC_VALUE and TLSDESC
  ...
  HOWTO (R_XTENSA_GOTTLSDESC, 0, 4, 32, false, 0,
	 complain_overflow_bitfield, bfd_elf_generic_reloc,
	 "R_XTENSA_GOTTLSDESC", false, 0, 0xffffffff, false),
  HOWTO (R_XTENSA_SYM32, 0, 4, 32, false, 0, complain_overflow_bitfield,
	 bfd_elf_generic_reloc, "R_XTENSA_SYM32",
	 false, 0, 0xffffffff, false),
  HOWTO (R_XTENSA_TLSDESC, 0, 4, 64, false, 0,
	 complain_overflow_bitfield, bfd_elf_generic_reloc,
	 "R_XTENSA_TLSDESC", false, 0, 0xffffffff, false),
};
...
struct elf_xtensa_link_hash_entry
{
  ...
  /* FDPIC: canonical descriptor ownership frozen during dynamic-section
     sizing.  Xtensa's late symbol localization can remove a dynamic index,
     so relocation emission must not reclassify the symbol afterwards.  */
  signed char funcdesc_dynamic;

  /* FDPIC: count of references that record an absolute address and
     therefore need one .rofixup table entry each in a
     position-dependent executable.  Descriptor words are counted
     separately, two per allocated descriptor slot.  */
  bfd_signed_vma rofixup_refcount;

  /* FDPIC: count of plain R_XTENSA_32/R_XTENSA_SYM32 locations in
     allocated program data.  These locations are a subset of the
     .rofixup count above, but need their own count for PIE/DSO links:
     descriptor words and GOT slots have separate dynamic-relocation
     sizing, whereas these locations do not.  */
  bfd_signed_vma abs_reloc_refcount;
  ...
  /* FDPIC: one writable 8-byte TLS descriptor in the module GOT for
     every TLS symbol referenced through GOTTLSDESC.  */
  bfd_signed_vma tlsdesc_refcount;
  bfd_vma tlsdesc_offset;
...
struct elf_xtensa_obj_tdata
{
  ...
  /* FDPIC: per-local-symbol count of plain absolute relocation sites
     that need an R_XTENSA_RELATIVE relocation in a PIE or DSO.  */
  bfd_signed_vma *local_abs_reloc_refcounts;
  ...
  /* FDPIC: per-local-symbol TLS descriptor counts and GOT offsets.  */
  bfd_signed_vma *local_tlsdesc_refcounts;
  bfd_vma *local_tlsdesc_offsets;
};
...
#define elf_xtensa_local_abs_reloc_refcounts(abfd) \
  (elf_xtensa_tdata (abfd)->local_abs_reloc_refcounts)
...
#define elf_xtensa_local_tlsdesc_refcounts(abfd) \
  (elf_xtensa_tdata (abfd)->local_tlsdesc_refcounts)
#define elf_xtensa_local_tlsdesc_offsets(abfd) \
  (elf_xtensa_tdata (abfd)->local_tlsdesc_offsets)
...
struct elf_xtensa_link_hash_table
{
  ...
  /* FDPIC read-only fixup table.  Position-dependent executables
     record every location that holds an absolute address here; the
     libc startup code walks the table at load time and relocates the
     recorded values through the process loadmap.  Position-
     independent executables use .rel.dyn instead.  */
  asection *srofixup;
  ...
  /* Mixing the normal Xtensa ELF ABI with FDPIC is never valid.  Track
     inputs explicitly instead of inferring the state from the output OSABI:
     the latter made the diagnostic depend on input-file order.  */
  bool saw_fdpic_input;
  bool saw_nonfdpic_input;
};

/* uClibc's FDPIC startup contract reserves three words at the start of
   the module GOT.  Word zero is initialized by the static linker; the
   dynamic loader owns words one and two.  Keep the normal Xtensa ABI's
   four-byte header unchanged.  */
#define XTENSA_FDPIC_GOT_HEADER_SIZE 12
  ...
  if (entry != NULL)
    {
      ...
      eh->funcdesc_dynamic = -1;
      eh->rofixup_refcount = 0;
      eh->abs_reloc_refcount = 0;
  ...
  if (ind->root.type == bfd_link_hash_indirect)
    {
	 ...
	 and emits records for which no space was reserved.  */
      ...
      edir->abs_reloc_refcount += eind->abs_reloc_refcount;
      eind->abs_reloc_refcount = 0;
...
/* Return true when the FDPIC loader, rather than this link, must provide the
   canonical function descriptor.  A symbol present in the dynamic symbol
   table needs one process-wide descriptor even when the regular ELF symbol
   has subsequently been localized for direct references.  Using DYNDX also
   makes the decision stable between dynamic-section sizing and relocation;
   the generic interposition predicate can change as the link localizes
   symbols.  Ordinary data references must continue to use
   elf_xtensa_dynamic_symbol_p instead.  */

static inline bool
elf_xtensa_fdpic_dynamic_funcdesc_candidate_p
...
static inline bool
elf_xtensa_fdpic_dynamic_funcdesc_p (struct elf_link_hash_entry *h,
				     struct bfd_link_info *info)
{
  if (h != NULL)
    {
      signed char dynamic = elf_xtensa_hash_entry (h)->funcdesc_dynamic;

      if (dynamic >= 0)
	return dynamic != 0;
    }
  return elf_xtensa_fdpic_dynamic_funcdesc_candidate_p (h, info);
}
...

/* Create the FDPIC support sections on dynobj the first time an FDPIC
   relocation is seen.  They must exist before the linker script
   placement pass, and dynobj -- a normal input file for static links
   -- makes the script wildcards see them.  ABFD is the input file
   whose relocation triggered the creation.  */

static bool
elf_xtensa_create_fdpic_sections (bfd *abfd, struct bfd_link_info *info)
{
  ...
  htab->sgotfuncdesc =
    bfd_make_section_anyway_with_flags (htab->elf.dynobj, ".got.funcdesc",
						got_flags);
  if (htab->sgotfuncdesc == NULL
      || !bfd_set_section_alignment (htab->sgotfuncdesc, 2))
    return false;

  /* The descriptors' got_value word needs the module GOT base.  For
     fully static links the Xtensa backend has no .got yet: provide the
     three-word FDPIC loader header here.  Both the GOT and function
     descriptors must be writable while startup relocations are applied.  */
  if (htab->elf.sgot == NULL)
    {
      htab->elf.sgot =
	bfd_make_section_anyway_with_flags (htab->elf.dynobj, ".got",
					    got_flags);
      if (htab->elf.sgot == NULL
...
/* Only allocated program data is visited by an FDPIC startup relocator.
   Xtensa's .xt.* property tables also carry R_XTENSA_32 records, but they
   are static linker metadata at VMA zero rather than runtime pointers.
   Keep this predicate shared by the sizing and emission passes so their
   view of .rofixup can never diverge.  */

static bool
elf_xtensa_fdpic_runtime_fixup_section_p (asection *sec)
{
...
/* Count one absolute-address reference for symbol R_SYMNDX of ABFD.
   Global symbols count on their hash entry, local symbols in the
   per-object array, allocated on first use.  Each counted reference
   becomes one .rofixup table entry in a position-dependent link.  */

static bool
elf_xtensa_bump_rofixup (bfd *abfd, struct elf_link_hash_entry *h,
			 ...
			 unsigned r_symndx)
{
  if (h)
    ...
    {
      if (elf_xtensa_local_rofixup_refcounts (abfd) == NULL)
	{
	  ...
	  if (mem == NULL)
	    return false;
	  elf_xtensa_local_rofixup_refcounts (abfd)
	    = (bfd_signed_vma *) mem;
	}
      elf_xtensa_local_rofixup_refcounts (abfd) [r_symndx]++;
    }
  return true;
}

/* Count a plain absolute relocation site separately from the complete
   .rofixup inventory.  Dynamic FDPIC links emit one relocation for each
   such site, while descriptors and GOT slots use their own sizing paths.  */

static bool
elf_xtensa_bump_abs_reloc (bfd *abfd, struct elf_link_hash_entry *h,
			   struct elf_xtensa_link_hash_entry *eh,
			   unsigned r_symndx)
{
  if (h)
    eh->abs_reloc_refcount++;
  else
    {
      if (elf_xtensa_local_abs_reloc_refcounts (abfd) == NULL)
	{
	  bfd_size_type size = elf_tdata (abfd)->symtab_hdr.sh_info;
	  void *mem;
	  ...
	  if (mem == NULL)
	    return false;
	  elf_xtensa_local_abs_reloc_refcounts (abfd)
	    = (bfd_signed_vma *) mem;
	}
      elf_xtensa_local_abs_reloc_refcounts (abfd) [r_symndx]++;
    }
  return true;
}
...
/* Count a reference to a writable FDPIC TLS descriptor.  Repeated
   references to the same symbol share one two-word descriptor in the
   module GOT and one R_XTENSA_TLSDESC dynamic relocation.  */

static bool
elf_xtensa_bump_tlsdesc (bfd *abfd, struct elf_link_hash_entry *h,
			 struct elf_xtensa_link_hash_entry *eh,
			 unsigned r_symndx)
{
  if (h)
    ...
    {
      if (elf_xtensa_local_tlsdesc_refcounts (abfd) == NULL)
	{
	  bfd_size_type size = elf_tdata (abfd)->symtab_hdr.sh_info;
	  void *mem;
	  ...
	  if (mem == NULL)
	    return false;
	  elf_xtensa_local_tlsdesc_refcounts (abfd)
	    = (bfd_signed_vma *) mem;

	  mem = bfd_zalloc (abfd, size * sizeof (bfd_vma));
	  if (mem == NULL)
	    return false;
	  elf_xtensa_local_tlsdesc_offsets (abfd) = (bfd_vma *) mem;
	}
      elf_xtensa_local_tlsdesc_refcounts (abfd) [r_symndx]++;
    }
  return true;
}
			 ...
			 const Elf_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < rel_end; rel++)
	...
	case R_XTENSA_GOTTLSDESC:
	  /* The compiler emits only a GOT-relative offset.  The linker
	     allocates one writable two-word descriptor per TLS symbol and
	     the dynamic loader initializes it through R_XTENSA_TLSDESC.  */
	  if (elf_elfheader (abfd)->e_ident[EI_OSABI]
	      != ELFOSABI_XTENSA_FDPIC)
	    {
	      _bfd_error_handler
		(_("%pB: R_XTENSA_GOTTLSDESC requires an FDPIC input"), abfd);
	      bfd_set_error (bfd_error_bad_value);
	      return false;
	    }
	  if (!elf_xtensa_create_fdpic_sections (abfd, info)
	      || !elf_xtensa_bump_tlsdesc (abfd, h, eh, r_symndx))
	    return false;
	  continue;
	...
	case R_XTENSA_FUNCDESC:
	  ...
	  if (elf_xtensa_fdpic_runtime_fixup_section_p (sec)
	...
	case R_XTENSA_SYM32:
	  /* A plain 32-bit symbol reference in an FDPIC object.  It
	     needs a .rofixup entry in a static executable or a dynamic
	     relocation in a PIE/DSO.  */
	  if (!elf_xtensa_create_fdpic_sections (abfd, info))
	    return false;
	  if (elf_xtensa_fdpic_runtime_fixup_section_p (sec)
	      && (!elf_xtensa_bump_rofixup (abfd, h, eh, r_symndx)
		  || !elf_xtensa_bump_abs_reloc (abfd, h, eh, r_symndx)))
	    return false;
	  continue;
	...
	case R_XTENSA_GOTFUNCDESC:
	  ...
	  if (h)
	      ...
	      if (eh->gotfamily_refcount == 0)
		{
		  ...
		  /* An undefined weak reference is represented by a zero GOT
		     word.  It must stay zero after load, so it has neither a
		     descriptor nor a runtime fixup.  */
	...
	case R_XTENSA_32:
	  /* In an FDPIC object a plain 32-bit reference also holds an
	     absolute address at run time, so it takes a fixup entry
	     like the dedicated FDPIC relocations.  */
	  if (elf_elfheader (abfd)->e_ident[EI_OSABI]
	      == ELFOSABI_XTENSA_FDPIC)
	    {
	      if (!elf_xtensa_create_fdpic_sections (abfd, info))
		return false;
	      if (elf_xtensa_fdpic_runtime_fixup_section_p (sec)
		  && (!elf_xtensa_bump_rofixup (abfd, h, eh, r_symndx)
		      || !elf_xtensa_bump_abs_reloc (abfd, h, eh,
						   r_symndx)))
		return false;
	      continue;
	    }
	...
	case R_XTENSA_PLT:
	  /* R_XTENSA_PLT denotes the legacy Xtensa address-only PLT ABI.
	     An FDPIC call needs both words of a function descriptor: the
	     entry address and the callee GOT.  Reinterpreting this relocation
	     as a descriptor reference loses that distinction and can also
	     require a runtime relocation in an executable literal pool.  The
	     FDPIC compiler must use GOTFUNCDESC/GOTOFFFUNCDESC instead.  */
	  if (elf_elfheader (abfd)->e_ident[EI_OSABI]
	      == ELFOSABI_XTENSA_FDPIC)
	    {
	      _bfd_error_handler
		(_("%pB: R_XTENSA_PLT is not valid for Xtensa FDPIC; "
		   "use a function descriptor relocation"), abfd);
	      bfd_set_error (bfd_error_bad_value);
	      return false;
	    }
	  tls_type = GOT_NORMAL;
...
elf_xtensa_allocate_dynrelocs (struct elf_link_hash_entry *h, void *arg)
{
  ...
  if (eh->tlsdesc_refcount > 0)
    htab->elf.srelgot->size += sizeof (Elf32_External_Rela);

  if (bfd_link_pic (info))
    {
      bool dynamic = elf_xtensa_fdpic_dynamic_funcdesc_candidate_p (h, info);
      bfd_size_type rela_count = 0;

      if (eh->funcdesc_refcount > 0
	  || eh->funcdesc_reloc_count > 0
	  || eh->funcdesc_value_refcount > 0)
	eh->funcdesc_dynamic = dynamic;

      /* One descriptor slot is shared by every reference to a symbol.
	 A dynamic symbol needs one FUNCDESC_VALUE relocation for the whole
	 slot; a locally resolved descriptor has two absolute words.  Every
	 plain FUNCDESC site and every inline FUNCDESC_VALUE instance is
	 separate and must also be reserved.  Counting the first occurrence
	 only used to under-allocate .rela.got for repeated references.  */
      if (eh->funcdesc_refcount > 0)
	...
	+= rela_count * sizeof (Elf32_External_Rela);
    }
...
elf_xtensa_allocate_local_got_size (struct bfd_link_info *info)
{
  ...
  /* Position-independent FDPIC links reserve .rel.dyn space for
     local symbols: one R_XTENSA_RELATIVE record per FUNCDESC site,
     two per descriptor-referenced symbol, two per FUNCDESC_VALUE
     reference, and one per GOT slot.  */
  for (i = info->input_bfds; i; i = i->link.next)
    {
      bfd_size_type j, cnt = elf_tdata (i)->symtab_hdr.sh_info;

      if (elf_xtensa_local_funcdesc_refcounts (i) != NULL)
	for (j = 0; j < cnt; j++)
	  if (elf_xtensa_local_funcdesc_refcounts (i) [j] > 0)
	    htab->elf.srelgot->size
	      += 2 * sizeof (Elf32_External_Rela);

      if (elf_xtensa_local_funcdesc_reloc_counts (i) != NULL)
	for (j = 0; j < cnt; j++)
	  htab->elf.srelgot->size
	    += (elf_xtensa_local_funcdesc_reloc_counts (i) [j]
		* sizeof (Elf32_External_Rela));

      if (elf_xtensa_local_funcdesc_value_refcounts (i) != NULL)
	for (j = 0; j < cnt; j++)
	  htab->elf.srelgot->size
	    += (2 * elf_xtensa_local_funcdesc_value_refcounts (i) [j]
		* sizeof (Elf32_External_Rela));

      if (elf_xtensa_local_gotfamily_refcounts (i) != NULL)
	for (j = 0; j < cnt; j++)
	  if (elf_xtensa_local_gotfamily_refcounts (i) [j] > 0)
	    htab->elf.srelgot->size += sizeof (Elf32_External_Rela);

      if (elf_xtensa_local_tlsdesc_refcounts (i) != NULL)
	for (j = 0; j < cnt; j++)
	  if (elf_xtensa_local_tlsdesc_refcounts (i) [j] > 0)
	    htab->elf.srelgot->size += sizeof (Elf32_External_Rela);

      if (elf_xtensa_local_abs_reloc_refcounts (i) != NULL)
	for (j = 0; j < cnt; j++)
	  htab->elf.srelgot->size
	    += (elf_xtensa_local_abs_reloc_refcounts (i) [j]
		* sizeof (Elf32_External_Rela));
    }
}
...
/* Append one dynamic relocation to SREL.  OFFSET is the runtime
   address of the location being relocated; SYM is the dynamic symbol
   index (0 for symbol-less relocations); TYPE and ADDEND make up the
   relocation record.  For symbol-less relocations the slot contents
   convention follows the supported libcs: the location is pre-filled
   with its link-time value and the addend is zero, which the
   uClibc-ng, musl and glibc FDPIC startup paths all interpret
   correctly.  */

static void
elf_xtensa_add_dynreloc (bfd *output_bfd, asection *srel, bfd_vma offset,
			 int type, bfd_vma addend, unsigned long sym)
{
  Elf_Internal_Rela outrel;
  bfd_byte *loc;

  outrel.r_offset = offset;
  outrel.r_info = ELF32_R_INFO (sym, type);
  outrel.r_addend = addend;

  loc = srel->contents + srel->reloc_count * sizeof (Elf32_External_Rela);
  BFD_ASSERT ((srel->reloc_count + 1) * sizeof (Elf32_External_Rela)
	      <= srel->size);
...
/* Like elf_xtensa_fill_funcdesc_slot, but for symbols the dynamic
   linker resolves at run time: the descriptor words are filled by the
   loader, so this variant emits one R_XTENSA_FUNCDESC_VALUE record
   for the slot on its first use and returns the slot's runtime
   address.  */

static bfd_vma
elf_xtensa_fill_funcdesc_slot_dynamic (bfd *output_bfd,
				       struct bfd_link_info *info,
				       struct elf_xtensa_link_hash_table *htab,
				       bfd *input_bfd,
				       struct elf_link_hash_entry *h,
				       unsigned r_symndx)
{
  bfd_vma *pdesc = h
    ...
    : &elf_xtensa_local_funcdesc_offsets (input_bfd) [r_symndx];
  bfd_vma desc_addr = (htab->sgotfuncdesc->output_section->vma
		       + htab->sgotfuncdesc->output_offset
		       + (*pdesc & ~1));

  if (!(*pdesc & 1))
    {
      *pdesc |= 1;
      elf_xtensa_add_dynreloc (output_bfd, htab->elf.srelgot, desc_addr,
			      R_XTENSA_FUNCDESC_VALUE, 0,
			      h ? h->dynindx : 0);
      (void) info;
    }
  return desc_addr;
...
/* Append one entry to the FDPIC read-only fixup table.  OFFSET is the
   ...
   process loadmap.  The entry itself is a plain 32-bit address, which
   matches the table format both supported libcs expect, including the
   final entry that holds the GOT base value instead of a location
   address.  */
...
elf_xtensa_add_rofixup (bfd *output_bfd, asection *srofixup, bfd_vma offset)
{
  ...
  BFD_ASSERT (srofixup != NULL);
  entry_offset = srofixup->reloc_count * 4;
  BFD_ASSERT (entry_offset < srofixup->size);
  bfd_put_32 (output_bfd, offset,
	      srofixup->contents + entry_offset);
...
elf_xtensa_count_funcdesc (struct elf_link_hash_entry *h, void *data)
{
  ...
  if (eh->gotfamily_refcount > 0)
    {
      eh->gotfamily_offset = cd->got_size;
      cd->got_size += 4;
      cd->gotfamily_count++;
    }
  if (eh->tlsdesc_refcount > 0)
    {
      eh->tlsdesc_offset = cd->got_size;
      cd->got_size += 8;
      cd->tlsdesc_count++;
    }
  return true;
				  ...
				  struct elf_xtensa_link_hash_table *htab)
{
  ...
  cd.tlsdesc_count = 0;
  cd.got_size = XTENSA_FDPIC_GOT_HEADER_SIZE;
  cd.htab = htab;
  ...
  for (i = info->input_bfds; i; i = i->link.next)
	...
	for (j = 0; j < cnt; j++)
      ...
      /* GOT slots are assigned past the three-word FDPIC header.  */
	...
	for (j = 0; j < cnt; j++)
	  if (refcounts[j] > 0)
	    {
	      offsets[j] = cd.got_size;
	      cd.got_size += 4;
	      cd.gotfamily_count++;
	    }

      /* TLS descriptors are two-word GOT objects, not pointers that need
	 a fixup entry.  Their single dynamic TLSDESC relocation initializes
	 both words.  */
      refcounts = elf_xtensa_local_tlsdesc_refcounts (i);
      offsets = elf_xtensa_local_tlsdesc_offsets (i);
      if (refcounts != NULL)
	for (j = 0; j < cnt; j++)
	  if (refcounts[j] > 0)
	    {
  ...
  /* The GOT holds the three-word FDPIC header followed by the slot array.  The
     contents buffer is allocated here so the relocation pass can fill
     the slots.  */
  if (cd.got_size > XTENSA_FDPIC_GOT_HEADER_SIZE
      && htab->elf.sgot != NULL)
    {
      htab->elf.sgot->size = cd.got_size;
  ...
  /* Size the .rofixup table for position-dependent links: one
     table entry per absolute word -- one per SYM32/R_XTENSA_32/
     FUNCDESC reference, two per FUNCDESC_VALUE, two per descriptor
     slot, one per GOT slot -- plus the trailing GOT base entry.
     The position-independent reservation lives in
     elf_xtensa_allocate_dynrelocs /
     elf_xtensa_allocate_local_got_size, where the dynamic-symbol
...
elf_xtensa_late_size_sections (struct bfd_link_info *info)
{
  ...
  if (elf_hash_table (info)->dynamic_sections_created)
      ...
      if (bfd_link_executable (info) && !info->nointerp)
	{
      ...
      /* Preserve FDPIC slots sized above and reserve the complete loader
	 header.  Normal Xtensa links retain their historical one-word GOT
	 header.  */
      if (elf_elfheader (output_bfd)->e_ident[EI_OSABI]
	  == ELFOSABI_XTENSA_FDPIC)
	{
	  if (htab->elf.sgot->size < XTENSA_FDPIC_GOT_HEADER_SIZE)
	    htab->elf.sgot->size = XTENSA_FDPIC_GOT_HEADER_SIZE;
	}
      else
			     ...
			     asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
	  ...
	  && IS_XTENSA_TLS_RELOC (r_type) != (sym_type == STT_TLS))
	{
      ...
      /* FDPIC startup and dynamic relocations are stores.  Silently placing
	 one in an executable/read-only load segment works on MMU targets only
	 by temporarily changing page permissions; it is impossible for true
	 Flash XIP.  Make such code generation a link error instead of producing
	 an image that faults before main.  GOT-relative relocations are offsets
	 resolved by the static linker and therefore do not enter this check.  */
      if (elf_elfheader (output_bfd)->e_ident[EI_OSABI]
	  == ELFOSABI_XTENSA_FDPIC
	  && (input_section->flags & SEC_READONLY) != 0
	  && elf_xtensa_fdpic_runtime_fixup_section_p (input_section)
	  && (r_type == R_XTENSA_32
	      || r_type == R_XTENSA_SYM32
	      || r_type == R_XTENSA_FUNCDESC
	      || r_type == R_XTENSA_FUNCDESC_VALUE))
	{
	  error_message =
	...
	case R_XTENSA_GOTTLSDESC:
	  {
	    bfd_vma *pdesc = h
	      ? &elf_xtensa_hash_entry (h)->tlsdesc_offset
	      : &elf_xtensa_local_tlsdesc_offsets (input_bfd) [r_symndx];
	    bfd_vma desc_off = *pdesc & ~1;
	    bfd_vma got_base;
	    bfd_vma desc_addr;
	    ...
	    if (htab->elf.sgot == NULL)
	      {
		error_message = _("FDPIC TLS descriptor GOT not allocated");
		(*info->callbacks->reloc_dangerous)
	    ...
	    got_base = (htab->elf.sgot->output_section->vma
			+ htab->elf.sgot->output_offset);
	    desc_addr = got_base + desc_off;

	    if (!(*pdesc & 1))
	      {
		asection *srel = htab->elf.srelgot;
		unsigned long indx = h && h->dynindx != -1 ? h->dynindx : 0;
		bfd_vma addend;
		...
		if (!elf_hash_table (info)->dynamic_sections_created
		  ...
		  {
		    error_message =
		      _("FDPIC TLS descriptor requires dynamic sections");
		    (*info->callbacks->reloc_dangerous)
		      (info, error_message, input_bfd, input_section,
		       rel->r_offset);
		    continue;
		...
		/* A zero symbol index denotes a TLS offset in this module;
		   a dynamic symbol lets the loader resolve both module and
		   offset.  The descriptor itself is already zero-filled.  */
		addend = (indx == 0
			  ? relocation - dtpoff_base (info)
			  : rel->r_addend);
		elf_xtensa_add_dynreloc (output_bfd, srel, desc_addr,
					R_XTENSA_TLSDESC, addend, indx);
		*pdesc |= 1;
	      }
	...
	case R_XTENSA_SYM32:
	  /* A plain 32-bit symbol reference in an FDPIC object.  */
	case R_XTENSA_32:
	  /* Both forms hold an absolute address at run time: in a
	     position-dependent FDPIC executable the resolved location
	     is recorded in the fixup table before the common 32-bit
	     handling.  The Xtensa property and literal table sections
	...
	case R_XTENSA_PLT:
	  if (elf_hash_table (info)->dynamic_sections_created
	      ...
	      && (elf_elfheader (output_bfd)->e_ident[EI_OSABI]
		  != ELFOSABI_XTENSA_FDPIC
		  || elf_xtensa_fdpic_runtime_fixup_section_p (input_section))
	...
	case R_XTENSA_FUNCDESC:
	  {
	    ...
	    if (elf_xtensa_fdpic_dynamic_funcdesc_p (h, info))
	      {
		...
		if (!bfd_link_pic (info) || srel == NULL)
		  {
		    error_message =
		      _("FDPIC function descriptor for dynamic symbol "
			"in a non-dynamic link");
		    (*info->callbacks->reloc_dangerous)
		      (info, error_message, input_bfd, input_section,
		       rel->r_offset);
		    continue;
		...
		/* The descriptor words are resolved by the loader; the
		   slot fill emits the VALUE record on first use.  */
		elf_xtensa_fill_funcdesc_slot_dynamic
	...
	case R_XTENSA_GOTFUNCDESC:
	  {
	    ...
	    bfd_vma value;
	    bool weak_null = is_weak_undef;

	    if (weak_null)
	      value = 0;
	    else if (r_type == R_XTENSA_GOTFUNCDESC
		&& elf_xtensa_fdpic_dynamic_funcdesc_p (h, info))
	      value = elf_xtensa_fill_funcdesc_slot_dynamic
		(output_bfd, info, htab, input_bfd, h, r_symndx);
	    else if (r_type == R_XTENSA_GOTFUNCDESC)
	    ...
	    if (!(*pslot & 1))
	      {
		bool dynamic_data = (!weak_null
				     && r_type == R_XTENSA_GOT
				     && elf_xtensa_dynamic_symbol_p (h, info));
		bfd_vma slot_addr = (htab->elf.sgot->output_section->vma
				     + htab->elf.sgot->output_offset + slot);

		bfd_put_32 (output_bfd, dynamic_data ? 0 : value,
		...
		*pslot |= 1;
		...
		if (!weak_null
		    && elf_xtensa_fdpic_use_rofixup_p (info, htab))
		  elf_xtensa_add_rofixup (output_bfd, htab->srofixup,
					  slot_addr);
		else if (!weak_null && bfd_link_pic (info))
		  elf_xtensa_add_dynreloc (output_bfd, htab->elf.srelgot,
					  slot_addr,
					  dynamic_data ? R_XTENSA_GLOB_DAT
					  : R_XTENSA_RELATIVE,
					  0, dynamic_data ? h->dynindx : 0);
	      }
	    ...
	    relocation = slot;
	    rel->r_addend = 0;
	    /* Any undefined/preemptible symbol is now represented by the
	       loader relocation on its writable GOT/descriptor slot.  The
	       instruction literal itself is completely resolved.  */
	    unresolved_reloc = false;
	  }
	  break;
	...
	case R_XTENSA_GOTOFFFUNCDESC:
	  {
	    ...
	    if (r_type == R_XTENSA_GOTOFFFUNCDESC)
	      {
		if (elf_xtensa_fdpic_dynamic_funcdesc_p (h, info))
		  relocation = elf_xtensa_fill_funcdesc_slot_dynamic
		    (output_bfd, info, htab, input_bfd, h, r_symndx);
		else
		  relocation = elf_xtensa_fill_funcdesc_slot
		    (output_bfd, info, htab, input_bfd, h, r_symndx,
	    ...
	    relocation -= got_base;
	    rel->r_addend = 0;
	    /* A dynamic function descriptor slot carries the unresolved
	       symbol through its writable FUNCDESC_VALUE relocation.  The
	       GOTOFF site itself is now a fully resolved link-time offset.  */
	    unresolved_reloc = false;
	  }
	  break;
	      ...
	      if (!bfd_link_pic (info) || srel == NULL)
		{
	      ...
	      elf_xtensa_add_dynreloc (output_bfd, srel,
				      (input_section->output_section->vma
				       + input_section->output_offset
				       + rel->r_offset),
				      R_XTENSA_FUNCDESC_VALUE, rel->r_addend,
				      h->dynindx);
	      /* The words are loader-computed; write nothing.  */
	      bfd_put_32 (output_bfd, 0, contents + rel->r_offset);
	      bfd_put_32 (output_bfd, 0, contents + rel->r_offset + 4);
	      continue;
	    }
				    ...
				    bfd_byte *buf ATTRIBUTE_UNUSED)
{
  ...
  /* Finish the read-only fixup table with its trailing entry: the
     GOT base value, which the libc startup code relocates through the
     process loadmap and returns as the GOT value.  This must happen
     for position-dependent static links too, hence before the
     dynamic-section gate below.  */
  if (htab->srofixup != NULL)
    {
	...
	{
	  _bfd_error_handler
	    (_("%pB: internal error: .rofixup size mismatch "
	       "(%ld entries, %#" PRIx64 " bytes)"),
	     output_bfd, (long) htab->srofixup->reloc_count,
	     (uint64_t) htab->srofixup->size);
	  return false;
	}
    }
  ...
  if (sgot)
    {
      BFD_ASSERT (sgot->size
		  >= (elf_elfheader (output_bfd)->e_ident[EI_OSABI]
		      == ELFOSABI_XTENSA_FDPIC
		      ? XTENSA_FDPIC_GOT_HEADER_SIZE : 4));
      if (sdyn == NULL)
...
elf_xtensa_merge_private_bfd_data (bfd *ibfd, struct bfd_link_info *info)
{
  ...
  /* Propagate the FDPIC OSABI to the output and reject mixing FDPIC
     and non-FDPIC objects independently of their command-line order.  */
  htab = elf_xtensa_hash_table (info);
  if (htab == NULL)
    return false;
  input_fdpic = (elf_elfheader (ibfd)->e_ident[EI_OSABI]
		 == ELFOSABI_XTENSA_FDPIC);
  if (input_fdpic)
    htab->saw_fdpic_input = true;
  else
    htab->saw_nonfdpic_input = true;

  if (htab->saw_fdpic_input && htab->saw_nonfdpic_input)
    {
      _bfd_error_handler
	(_("%pB: cannot mix FDPIC and non-FDPIC Xtensa objects"),
	 ibfd);
      return false;
    }
  if (input_fdpic)
